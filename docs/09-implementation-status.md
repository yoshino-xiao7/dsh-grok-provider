...
- 包中不存在独立 OAuth client identity、device flow、refresh/revoke、Harness credential grant 或模式选择接口；ADR-0003 已由 ADR-0005 取代。
...
- Node `24.19.0` 完整构建/测试通过：40 项，38 pass、0 fail、2 项 Windows-only 在 macOS 按预期跳过并由 CI matrix 承接。
