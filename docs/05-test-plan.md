...
`1.0.0` 仍为候选，不能把上述 `0.1.11` 供应链证据复用为其发布证据。新增 open-page/reasoning 聚焦回归 40/40、完整 Node 24 suite（245 项、243 pass、0 fail、2 项平台跳过）、生产依赖审计与 72 项干净 dry-run pack 已通过；macOS/Windows CI、最终提交与唯一制品仍需关闭。脱敏真实 Web/X 探针只记录事件类别、计数、闭合结果与错误位置，不保存 URL、检索/回复内容、prompt 或凭据。冻结精确 tarball 后仍需单独取得该制品的发布授权，再验证 Registry、GitHub Release、签名与 provenance 逐字节绑定。
