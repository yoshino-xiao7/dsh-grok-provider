...
- Web lifecycle、四项 X custom-tool 名称、Web+X 交错与 Web+function 混合都必须闭合；未启用类别、未知 name/event/annotation、重复 output index、乱序、截断或未闭合状态失败关闭。结构化 citation 有界校验后丢弃，任一 Search 出现后 encrypted reasoning replay 为空。`0.1.11` 历史回归继续锁定首次 Search-backed reasoning ID 复用与 raw `reasoning_text` / summary lifecycle 互斥；`1.0.0` 候选再锁定同一 ID 的后续多段复用：首次仍要求旧段已 `output_item.done` 且两段间有 completed Web/X Search，后续只允许相同 ID/type 的严格空 visible summary/content、无 summary/raw lifecycle、逐段 `output_item.done` 占位。可选 `encrypted_content` 只接受有界 opaque 字符串；terminal 未知键、accessor、非空内容、未闭合段，以及 `response.incomplete` 仍有 open 复用段时全部失败关闭；另以正例锁定闭合复用段后的 max-token 终态。
...
`1.0.0` 仍为候选，不能把上述 `0.1.11` 供应链证据复用为其发布证据。新增 open-page/reasoning 聚焦回归 40/40、完整 Node 24 suite（245 项、243 pass、0 fail、2 项平台跳过）、生产依赖审计与 72 项干净 dry-run pack 已通过；代码 PR #28 已合入 `yukiryou/main@7a6364dd58f3c7e9e1ad68a3d0197a14254bcb8c`，main CI run `33308371009` 的 macOS/Windows 均全绿。发布证据 final CI、最终提交与唯一制品仍需关闭。脱敏真实 Web/X 探针只记录事件类别、计数、闭合结果与错误位置，不保存 URL、检索/回复内容、prompt 或凭据。冻结精确 tarball 后仍需单独取得该制品的发布授权，再验证 Registry、GitHub Release、签名与 provenance 逐字节绑定。
