...
`1.0.2` 的验收事实必须逐项产生后再填写。代码语义门禁至少包括空 reasoning 零可见 block、非空 reasoning 延迟启动、混合多项顺序、严格失败关闭和旧行为不回归；发行门禁还须检查实际 tarball 内的双语 README 使用 `@1.0.2`，不能仅检查发布后主分支。未完成 CI、冻结制品、摘要、授权、Release、Registry、签名、attestations 或 provenance 前不得预写为已完成。
