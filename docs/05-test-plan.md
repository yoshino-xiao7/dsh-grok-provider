...
- Web lifecycle、四项 X custom-tool 名称、Web+X 交错与 Web+function 混合都必须闭合；未启用类别、未知 name/event/annotation、重复 output index、乱序、截断或未闭合状态失败关闭。结构化 citation 有界校验后丢弃，任一 Search 出现后 encrypted reasoning replay 为空。`0.1.11` 另外锁定：reasoning ID 只有在旧段已闭合、一个 server Search 已完成且位于新旧 output index 之间、新段严格为空时才允许复用一次；raw `reasoning_text` 与 summary lifecycle 互斥；replay 元数据不保存 raw 明文，后续请求只发送 encrypted content 与空 summary，而当前流 raw delta 保持 Harness 可见。
