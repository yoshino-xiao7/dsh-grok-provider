...
`1.0.1` 当前只是源码候选。上述同名冲突、receipt、SSE 错误归因、一次脱敏真实结构回放、精确 Node `24.19.0` 本地全量测试（253 tests、251 pass、0 fail、2 platform skips）、生产依赖审计（0 漏洞）、确定性 build、隔离 cache 的 73 文件 dry-run pack 与秘密模式扫描已完成；扫描只命中显式 fixture canary 及其检查表记录。代码 PR #31 的 push/PR CI、merge commit `0c60200e12c3b8455331f31a317ece9b1945c458` 及 main CI run `33312621786` 也已在 macOS 14 / Windows 2022 全绿。进入发布授权前仍必须形成固定 final release commit，完成其双平台 CI、候选 tarball 冻结与 Node 24 隔离安装。随后向仓库所有者展示固定 commit、文件数/大小、SHA-1/SHA-256/SRI 和测试证据，并另行取得对该精确制品的发布授权；GitHub Release、Trusted Publisher、Registry 字节回读、signature、attestations 与 provenance 均须在发布后独立关闭。Windows 真机浏览器登录仍是单独边界，不得从该修复或 CI 推断。
