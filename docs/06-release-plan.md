...
下一版本固定为 `1.0.0`，但当前仍是候选开发而非可发布制品。候选范围只修复剩余 Search response 形状：已完成 server Search 支撑的 reasoning ID 可以继续出现为多个严格空、逐次闭合的占位 lifecycle；完成态 Web Search 可以返回精确有界的 `open_page` type/URL，且 streamed/final action 必须一致。严格空要求 visible summary/content 与 summary/raw lifecycle 为空，允许有界 opaque `encrypted_content`；每次复用必须到达自己的 `response.output_item.done`，仅当 `response.incomplete` 到来时仍有复用段未闭合才失败关闭，所有复用段闭合后的 `max_output_tokens` 终态仍有效。非空、跨类型、未知 terminal 或 accessor-backed 字段继续拒绝。Provider 校验 `open_page` 后立即丢弃 URL，不访问、不预览、不下载、不 replay。

最终源码的真实账号复验已脱敏完成：原始 Web/X 协议各完成 1 次、各 64 events，并观察到对应 Search；生产 adapter 共完成 5 次 Responses，direct Web/X 均为 `stop`，Harness 同名 `x_search` 三轮依次为 `tool-calls`、`tool-calls`、`stop`，前两轮各 1 次本地调用。探针不保存结果、URL、prompt、身份或凭据，也不替代自动化、双平台 CI、OAuth 或 Windows 真机验收。代码 PR #28 已合入 `yukiryou/main@7a6364dd58f3c7e9e1ad68a3d0197a14254bcb8c`，main CI run `33308371009` 双平台全绿；发布前仍必须依次完成发布证据 final CI、最终 release commit、唯一 tarball 冻结、摘要/SRI/隔离安装、仓库所有者对该精确制品的明确授权、不可变 tag/GitHub Release/Trusted Publisher，以及 Registry 字节、signature、attestation 与 provenance 回读。任何发布门禁完成前，npm `latest` 与安装文档均继续以 `0.1.11` 为稳定事实。
