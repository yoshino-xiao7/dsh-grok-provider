...
下一版本固定为 `1.0.0`，但当前仍是候选开发而非可发布制品。候选范围只修复剩余 Search response 形状：已完成 server Search 支撑的 reasoning ID 可以继续出现为多个严格空、逐次闭合的占位 lifecycle；完成态 Web Search 可以返回精确有界的 `open_page` type/URL，且 streamed/final action 必须一致。严格空要求 visible summary/content 与 summary/raw lifecycle 为空，允许有界 opaque `encrypted_content`；每次复用必须到达自己的 `response.output_item.done`，incomplete、非空、跨类型、未知 terminal 或 accessor-backed 字段继续失败关闭。Provider 校验 `open_page` 后立即丢弃 URL，不访问、不预览、不下载、不 replay。

最终源码的真实账号协议探针已脱敏完成：Web 为 5 个完成 Search / 206 events / `stop`，direct X 为 1 个官方 custom call / 91 events / `stop`，Harness 形状的两次 `x_search` 加第三次续写在 3 个 Responses 调用中闭环。探针不保存结果、URL、prompt、身份或凭据，也不替代自动化、双平台 CI、OAuth 或 Windows 真机验收。manifest/lock 版本更新与本地自动化已完成；发布前仍必须依次完成双平台 CI、最终 release commit、唯一 tarball 冻结、摘要/SRI/隔离安装、仓库所有者对该精确制品的明确授权、不可变 tag/GitHub Release/Trusted Publisher，以及 Registry 字节、signature、attestation 与 provenance 回读。任何发布门禁完成前，npm `latest` 与安装文档均继续以 `0.1.11` 为稳定事实。
