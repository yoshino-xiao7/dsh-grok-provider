...
当前可用 npm 稳定基线为 `dsh-grok-provider@1.0.0`，npm `latest=1.0.0`。最终 release commit 为 `c6548199582b122f1d285422eabea0205eaf602f`；annotated tag object `192561cda1ac58cbc4077f0de8fa614dff9a5557` peel 到该提交，final CI run [`33308603394`](https://github.com/yoshino-xiao7/dsh-grok-provider/actions/runs/33308603394) 双平台全绿。仓库所有者明确授权的唯一 tarball 含 72 个文件、226,704 bytes，unpacked size 为 715,014 bytes；SHA-1 为 `50e5d898dba241d1e19def7705db216e3060b892`，SHA-256 为 `30cd83dad77f7d2611126b3c4737c8fabffeae79f385fa623e61dcecfe39f5e2`，npm SRI 为 `sha512-WL2f6Kfg5yT5nNf1p4//mLSajCnZttL/pDR3BISrFgSGtZd9DEJlnibq08ETz503n1wHIdCBcU/ICMPG9K4vOw==`。Trusted Publisher run [`33309083806` attempt 1](https://github.com/yoshino-xiao7/dsh-grok-provider/actions/runs/33309083806/attempts/1) 已完成；冻结候选、GitHub Release 与 npm Registry tarball 逐字节一致。Node 24 Registry 隔离安装的 Host `name`/`apply` 与 client `id` smoke 通过；本包公开 metadata 包含 1 个 Registry signature、2 个 attestations，`npm audit signatures` 验证安装图中 11 个 signed packages 与 2 个 attested packages，SLSA provenance 精确绑定 `release.yml`、`v1.0.0`、release commit 与 publish run。

当前源码 manifest/lock 已进入尚未发布的 `1.0.1` 候选。该候选修复启用 xAI server Search 时，Harness 同名 `web_search` / `x_search` function definitions 与 server descriptors 共存导致的 fixed Proxy HTTP 400，并让 source transport error 保持为 `PROVIDER_ERROR`。本地精确 Node `24.19.0` 全量门禁为 253 tests、251 pass、0 fail、2 platform skips，生产依赖审计为 0 漏洞，73 文件 dry-run pack 与秘密模式扫描通过；一次授权的脱敏原失败 X 会话结构回放以 314 events 和 `response.completed` 结束。代码 PR #31、merge commit `0c60200e12c3b8455331f31a317ece9b1945c458` 与 main CI run `33312621786` 的双平台门禁已完成。固定 final release commit、其双平台 CI、候选 tarball 冻结与隔离安装、精确制品发布授权、tag/Release/Registry 及供应链回读仍未完成。
