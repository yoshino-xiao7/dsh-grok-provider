...
已发布 `0.1.7` 只增加 Windows 运行时诊断、登录失败可解释性与 `IconThinkOutline16` 设置导航图标维护。曾发布后撤回的 sidebar quota `0.1.8` 同样不修改图片边界；该 npm 版本号已消耗且不承载 Search。已发布 `0.1.9` 独立增加默认关闭的 Web/X Search 协议与页面，当前 `0.1.10` 候选只修复其 settings 集成；两者都不修改上述图片模型集合、attachment 投影、资源上限或图片 wire。
