...
- 真实 SettingsProvider/LLM 回归、真实页面可写 smoke、候选隔离安装与候选双平台 CI 已经完成；发布前仍需关闭代码 PR、main CI 与最终唯一制品门禁。
