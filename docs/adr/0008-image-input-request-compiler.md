...
`prompt_cache_key` 不进入 compiler 或 `0.1.4`。它会引入会话标识隐私与 server-affinity 语义；“上游拒绝后去掉字段再试”还会重放已经开始的 POST，违反失败后不自动改变请求并重放的边界。ADR-0011 从 `1.0.3` 起只为响应体读取前的 401/403 增加一次官方 CLI 会话刷新与同请求重试；不允许删除字段或重放已开始的流。若以后需要 cache key，必须独立 ADR，并设计首次发送前可判定的能力或明确幂等策略。
