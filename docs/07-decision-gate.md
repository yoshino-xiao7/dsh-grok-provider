...
采用：协议 spike、全部安全测试、macOS arm64 与 Windows x64 预发布真实 smoke、服务条款复核和 npm provenance 发布链配置通过后，才允许发布。发布后必须完成 provenance attestation、Registry 回读和两平台生产 inspector 验收，之后才能进入 marketplace/curated 分发。
