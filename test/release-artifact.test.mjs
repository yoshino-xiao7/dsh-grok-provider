...
test("the exact 0.1.8 manifest exports runtime artifacts and Web loader metadata", async () => {
  ...
  assert.match(chineseReadme, /当前源码候选版本为 `0\.1\.8`/u)
  assert.match(chineseReadme, /dsh-grok-provider@0\.1\.8/u)
  ...
  assert.match(englishReadme, /current source candidate is `0\.1\.8`/u)
  assert.match(englishReadme, /dsh-grok-provider@0\.1\.8/u)
  ...
  assert.match(securityPolicy, /源码候选版本 `0\.1\.8`/u)
  assert.match(securityPolicy, /source candidate is `0\.1\.8`/u)
  ...
  assert.doesNotMatch(releaseNotes, /^# .*0\.1\.8/mu)
