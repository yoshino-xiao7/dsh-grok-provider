...
当前稳定版本为 `0.1.1`；`0.1.2-rc.1` 是 Windows 登录验收预发行版。DeepSeek Harness、Node.js 与操作系统仍按发布线明确维护；Grok CLI 不再用完整版本字符串作为信任门禁，而是严格校验官方默认路径、命令能力、生产 OIDC 凭据契约和固定服务端协议。macOS arm64 已完成真实验收；只有 Windows x64 预发行真机验收通过后，才会准备稳定 `0.1.2`。
