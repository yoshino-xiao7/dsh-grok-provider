...
当前候选版本为 `0.1.3`。DeepSeek Harness、Node.js 与操作系统按发布线明确维护；Grok CLI 不使用完整版本字符串作为信任门禁，而是严格校验官方默认路径、命令能力、生产 OIDC 凭据契约和固定服务端协议。macOS arm64 已完成真实验收；Windows x64 有代码与 CI 覆盖。`0.1.3` 只修复有界历史调用 ID 的请求内安全映射，不改动认证、凭据或平台执行边界。项目不再发行预发行版，安全或兼容性缺陷使用新的递增稳定版本修复。
