...
`1.0.0` 是尚未发布的候选修复，不属于当前支持稳定版，也未获得精确制品发布授权。候选把 reasoning ID 复用收窄为 Search-backed 且严格空的完整生命周期：可见 summary/content 及 summary/raw lifecycle 必须为空，允许有界 opaque `encrypted_content`；每次复用都必须到达自己的 `response.output_item.done`，截断、非空、跨类型、未知 terminal 字段或 accessor 字段继续失败关闭。完成态 Web Search `open_page` 只允许精确且有界的 `type + url`，streamed/final action 的类型与 URL 必须一致，校验后 URL 被丢弃且不会被访问、预览、下载或回放。最终源码的真实账号探针只记录计数与终止状态：Web 为 5 个完成 Search / 206 events / `stop`，direct X 为 1 个官方 custom call / 91 events / `stop`，Harness 形状的两次 `x_search` 加第三次续写为 3 个完成 Responses；未保存结果、URL、prompt、身份或凭据。版本与本地自动化已完成，双平台 CI、唯一制品、摘要/SRI、授权、发布与 Registry/signature/attestation/provenance 回读仍待完成。
