...
Candidate security note: `1.0.0` is not yet a supported or published release and has no exact-artifact authorization. Its planned Search-response repair accepts only complete Search-backed strictly empty reasoning reuse (empty visible summary/content and no summary/raw lifecycle, with bounded opaque `encrypted_content` allowed) and exact bounded completed `open_page` type/URL pairs whose streamed and final values agree. Every reuse requires its own `response.output_item.done`; incomplete, non-empty, cross-type, unknown-terminal, or accessor-backed shapes fail closed. Redacted probes retained only counts and terminal status, never results, URLs, prompts, identity, or credentials. Versioning, tests, CI, artifact freezing, publication, and Registry integrity/provenance readback remain pending.
