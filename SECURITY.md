...
本安全策略对应当前稳定版本 `0.1.9`。`0.1.8` 曾发布后撤回，npm 版本号不可复用；npm Registry 的 `latest` 已指向 `0.1.9`。DeepSeek Harness、Node.js 与操作系统按发布线明确维护；Grok CLI 不使用完整版本字符串作为信任门禁，而是严格校验官方默认路径、命令能力、生产 OIDC 凭据契约和固定服务端协议。图片发送已在真实 Harness 对话中确认可用；`0.1.9` Search 已完成失败关闭 fixture 下的隔离 Web Harness `ctx.llm` 四场景验收。macOS 14 / Windows 2022 CI 已通过，但浏览器手工对话、OAuth、真实账号/真实 xAI Search 请求和网络可达 Windows 真机外部浏览器弹出不在该隔离验收覆盖范围内。
...
English summary: GitHub Private vulnerability reporting is enabled and preferred. The current stable release is `0.1.9`; npm Registry `latest` points to `0.1.9`. Version `0.1.8` was published and then withdrawn, and npm version numbers cannot be reused. Bounded JPEG/PNG input with fixed `detail:"high"` remains enabled only for exact `grok-4.6`; browser sign-in diagnostics remain closed and redacted, and the official CLI still owns URL generation and browser launch. Version `0.1.9` adds independent, default-off Web/X Search only for exact `grok-4.6`. Its four isolated Web Harness `ctx.llm` cases and macOS 14 / Windows 2022 CI passed under fail-closed fixtures; this does not cover manual browser conversations, OAuth, a real account, real xAI Search requests, or network-reachable Windows browser launch. When enabled, prompts and model-derived queries reach xAI and may incur additional usage; results and citations are untrusted remote data. The provider accepts only the fixed-Proxy Web lifecycle and four verified X custom-tool names, emits no Harness-local tool calls for server search, never follows citation URLs, and suppresses encrypted reasoning replay after Search. Background-purpose calls, unverified models, disabled categories, unknown events, and incomplete lifecycles fail closed. Fixed path/argv, no-shell, credential, origin, and URL-rejection boundaries remain unchanged; no image generation, new auth mode, or custom endpoint is added. Never send credentials, identity data, prompts, tool arguments, cookies, image bytes, Search contents, or unreviewed diagnostics.
