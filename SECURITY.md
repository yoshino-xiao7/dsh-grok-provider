...
当前稳定版本为 `0.1.2`。DeepSeek Harness、Node.js 与操作系统按发布线明确维护；Grok CLI 不使用完整版本字符串作为信任门禁，而是严格校验官方默认路径、命令能力、生产 OIDC 凭据契约和固定服务端协议。macOS arm64 已完成真实验收；Windows x64 有代码与 CI 覆盖，但 `0.1.2` 未完成独立真机验收。项目不再发行预发行版，安全或兼容性缺陷使用新的递增稳定版本修复。
