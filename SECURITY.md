...
本安全策略对应源码候选版本 `0.1.8`；npm Registry 的 `latest` 与最近发布的稳定版本均为 `0.1.7`。DeepSeek Harness、Node.js 与操作系统按发布线明确维护；Grok CLI 不使用完整版本字符串作为信任门禁，而是严格校验官方默认路径、命令能力、生产 OIDC 凭据契约和固定服务端协议。macOS arm64 已完成真实验收，`0.1.6` 的图片发送也已在真实 Harness 对话中确认可用；Windows x64 有代码与 slow-fake 覆盖，正式候选仍需通过 Windows CI，且网络可达 Windows 真机外部浏览器弹出仍未完成验收。
...
English summary: GitHub Private vulnerability reporting is enabled and is the preferred reporting channel. The current source candidate is `0.1.8`; npm Registry `latest` and the most recent stable release are `0.1.7`. The candidate adds only a Provider-aware sidebar projection of the existing sanitized quota/reset data. Executable paths, stderr, environment, proxy data, OAuth credentials, account identity, and raw billing responses do not cross the Host boundary. Windows code and slow-fake coverage are present, while network-reachable external-browser launch still lacks real-device acceptance. Fixed path/argv, no-shell, credential, origin, and URL-rejection boundaries remain unchanged; no Search, generation, new auth mode, payment entry, or custom endpoint is added. Never send credentials, identity data, prompts, tool arguments, cookies, image bytes, or unreviewed diagnostics.
