...
- Package the bilingual README with durable `1.0.2` wording and an exact `dsh-grok-provider@1.0.2` installation command. The exact Node `24.19.0` local suite passes with 265 tests, 263 pass, 0 fail, and 2 platform skips; production audit reports zero vulnerabilities and the isolated-cache dry-run pack lists 74 files. Code PR #34, branch push/PR CI, merge commit `47d688cc47bc9643f3477ee9333cfdf7788045cd`, and main CI run `33318245251` passed on macOS 14 and Windows 2022. A fixed final release commit and CI, frozen artifact, isolated installation, publication, and supply-chain readback remain pending and are recorded only after they actually exist.
