...
- Add focused protocol regressions for multiple empty reuses, Web/X Search-backed reuse, lifecycle completion, opaque encrypted content, terminal/accessor rejection, and `open_page` start/final consistency.
- Complete redacted real-account probes through the production codec without retaining prompts, results, URLs, account identity, or credentials: the final-source Web Search run completed 5 server calls across 206 events and ended with `stop`; the final-source direct X Search run completed 1 official custom call across 91 events and ended with `stop`; a Harness-shaped flow with two `x_search` calls plus a third continuation completed in 3 Responses calls.
- Keep npm `latest` and the supported stable release at `0.1.11` until the `1.0.0` version, tests, dual-platform CI, frozen artifact, exact owner authorization, publication, Registry readback, signatures, attestations, and provenance are complete. Real-device Windows external-browser launch remains unverified.
